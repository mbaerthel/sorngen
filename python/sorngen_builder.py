...
def insertRegisterToSST(SST, registerLocation):
	...
	## 1/ pipeline register insertion engine
...
def createHDLNetsFromSST(HDL, SST):
    ...
    ## 1/ extract nets form SST
    print("creating nets...",end="")
    for cInst in HDL.instances:
        # 1.1/ get corresponding SST node
        cNode = cInst.nodeSST
        ## 2/ binary operator
        if cNode.type == sst.typeNode.BINOP:
            
            # 2.1/ port counter, handles different occurrences of numeric values
            pCTR = 0

            # 2.2/ get left sibling instance of actual node
            ...
            # 2.3/ get right sibling instance of actual node
        ...
        if (cNode.type == sst.typeNode.FUNCTION) or (cNode.type == sst.typeNode.REGISTER):
            ...
            # 3.1/ get sibling instances of actual node
            sNode = cNode.siblings[0]
            sInst = sNode.instHDL
            newNet = dp.sornNet(HDL, sNode.name+"_to_"+cInst.name)
            dp.register(newNet, cInst.ports[0])

            # 3.2/ assign to instance of output port
            if sNode.type == sst.typeNode.VARIABLE: continue
            for it in sInst.ports:
                if not it.isInput:
                    dp.register(newNet, it)
            else:
                # 3.2.1/ iterate through global ports and compare name
                for it in HDL.ports:
                    if not it.isGlobal: continue
                    if (it.name == sNode.name):
                        dp.register(newNet, it)

        ## 4/ connect global outputs
        if cNode.parent.isRoot:
            newNet = dp.sornNet(HDL, cNode.name+"_to_"+cNode.SST.rootNode.name)
            for it in cInst.ports:
                if not it.isInput:
                    dp.register(newNet, it)
            # 4.1/ iterate through global ports and compare name
            for it in HDL.ports:
                if not it.isGlobal: continue
                if (it.name == cNode.SST.rootNode.name):
                    dp.register(newNet, it)
...
def generateFunctionTable(env):
    ...
    for cHDL in env.HDL:
        for cInst in env.HDL[cHDL].instances:
            ...
            ## 2/ determine the kind of operation (one or two operands)
            if len(re.findall(r"<op\d*>",cInst.function)) == 2: 
                ...
                ## 3/ search for operation with keyword such as "sqrt" or "log" or "log2"
